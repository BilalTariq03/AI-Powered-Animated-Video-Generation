...
import mcp  # noqa: F401 — triggers all MCP tool registrations
...
from agents.orchestrator.graph import workflow, WritersRoomState
...
def build_phase2_handoff(script: dict, characters: list) -> dict:
    voice_configs = [
        {
            ...
            "voice_style":    char.get("voice_profile", {}).get("voice_style", "neutral"),
            "speaking_speed": char.get("voice_profile", {}).get("speaking_speed", "normal"),
            "pitch":          char.get("voice_profile", {}).get("pitch", "medium"),
            "tts_description":char.get("voice_profile", {}).get("tts_description", ""),
            "emotion_range":  char.get("voice_profile", {}).get("emotion_range", ["neutral"])
        }
    ...
    music_moods = [
        {"scene_id": s["scene_id"], "mood": s.get("mood","neutral"),
         "tone": s.get("tone","neutral"), "duration_seconds": s.get("duration_seconds", 30)}
...
def save_outputs(state: WritersRoomState):
    ...
    unified = {"story": script.get("story", {}), "scenes": script.get("scenes", []), "characters": characters}
    with open(MANIFEST, "w") as f:
        json.dump(unified, f, indent=2)
    print(f"\n[SAVED] {MANIFEST}  (unified PhaseOneOutput)")

    with open(CHAR_DB, "w") as f:
        json.dump({"total": len(characters), "characters": characters}, f, indent=2)
    ...
    print(f"[SAVED] {PHASE2_HANDOFF}  ({len(phase2['segments'])} segments, {len(phase2['voice_configs'])} voices)")

    phase3 = build_phase3_handoff(script, characters)
    with open(PHASE3_HANDOFF, "w") as f:
        json.dump(phase3, f, indent=2)
    print(f"[SAVED] {PHASE3_HANDOFF}  ({len(phase3['scenes'])} scenes)")

    images = state.get("images", [])
    if images:
        print(f"[SAVED] {len(images)} images in {IMAGES_DIR}/")
        for img in images:
...
def get_user_input() -> WritersRoomState:
    ...
    if choice == "1":
        ...
        print("\nManual Mode: Paste your script below. (Type END on a new line when done)\n")
        lines = []
        while True:
            line = input()
...
def main(prompt: str | None = None, auto: bool = False):
    ...
    if final_state.get("status") == "complete":
        ...
        print(f"[ERROR] Pipeline failed: {final_state['error']}")
